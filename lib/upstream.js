...
// per-request `dispatcher`, which is how an upstream would be routed through
// an outbound proxy if one were ever configured. It is the same
// implementation Node ships internally.
...
// A ProxyAgent is only needed when an upstream is explicitly configured to
// go through an outbound proxy (unnecessary on Toolforge — see config.js),
// so agents are built lazily and cached per URL.
